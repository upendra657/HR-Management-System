...
And it applies the migration, rolls it back, and applies it again. That step
earned its keep: Alembic's autogenerated `downgrade()` dropped the tables but
left the seven Postgres enum types behind, because a type is a schema object in
its own right and isn't owned by any table. The rollback looked fine and the
*next* upgrade died on `CREATE TYPE role_enum` already existing — so a
downgrade quietly made the database impossible to migrate forward again.
Nothing on SQLite can catch that; it has no native enum types.
